# All the things needed to encode and HLS streams
import args
import definitions
import helpers
...
def create_rendition(job):
    ...
    # Holder for the m3u8 file contents.
    output_playlist = "#EXTM3U\n#EXT-X-VERSION:3\n"
    helpers.log(job, "Creating renditions...")
    helpers.log(
        job,
        "Profile in use is '{}'.".format(
            job["commands"]["hls"]["hls_args"]["hls_profile"]
        ),
    )
    for rendition in definitions.encoder_profiles[
        job["commands"]["hls"]["hls_args"]["hls_profile"]
    ]:
        helpers.log(
            job,
        ...
        this_rendition["maxrate"] = this_rendition["maxrate"].format(
            int(
                ...
                * job["commands"]["hls"]["hls_args"]["hls_bitrate_ratio"]
            )
        )
        this_rendition["bufsize"] = this_rendition["bufsize"].format(
            int(
                float(rendition["bitrate"])
                * job["commands"]["hls"]["hls_args"]["hls_buffer_ratio"]
            )
        )
        ...
        print(this_rendition["hls_segment_filename"])
...
def create_hls(job):
    ...
    job["commands"]["hls"]["options"]["g"] = str(
        kf_interval * job["info"]["arguments"]["hls_keyframe_multiplier"]
    )

    # Set the Constant Rate Factor to normalize the bitrate throughout for easier streaming.
    job["commands"]["hls"]["options"]["crf"] = str(job["info"]["arguments"]["hls_crf"])

    # Set the length of each h.264 TS segment file.
    job["commands"]["hls"]["options"]["hls_time"] == str(
        job["info"]["arguments"]["hls_segment_duration"]
    )

    # The main input file
    job["commands"]["hls"]["input_options"]["i"] = job["source"]["input"]["filename"]
    ...
    job["commands"]["hls"]["hls_args"]["hls_profile"] == str(
        job["info"]["arguments"]["hls_profile"]
    )

    # These two values help us fine-tune the HLS encoding.
    job["commands"]["hls"]["hls_args"]["hls_bitrate_ratio"] == str(
        job["info"]["arguments"]["hls_bitrate_ratio"]
    )
    job["commands"]["hls"]["hls_args"]["hls_buffer_ratio"] == str(
        job["info"]["arguments"]["hls_buffer_ratio"]
    )

    # Create template entries for each of the encoder renditions specified in the profile.
    job, renditions_options, encoder_renditions, output_playlist = create_rendition(job)
    ...
    job["commands"]["hls"]["command"] = "ffmpeg " + convert_ffmpeg_command
...
def create_master_playlist(job, output_playlist):
    ...
    helpers.log(
        job,
        "Creating m3u8 playlist file {0}".format(
            str(job["commands"]["hls"]["output_options"]["directory"])
            + "/playlist.m3u8"
        ),
    )
    if not job["info"]["arguments"]["simulate"]:
        ...
        helpers.log(
            job,
            "Created m3u8 playlist file {0}".format(
                str(job["commands"]["hls"]["output_options"]["directory"])
                + "/playlist.m3u8"
            ),
        )
