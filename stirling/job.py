...
class StirlingJobOptions(StirlingClass):
    ...
    Attributes:
        ...
        job_file (pathlib.Path | str): A JSON document describing the job.json. This
            file is created when the job.json is created, and updated as the job.json
            progresses. Alternately, a job.json can be created from a JSON file
            ...
            a file named 'job.json.json' to be created in the output directory.
        log_file (pathlib.Path | str): The filename to output logs from the job.json.
        input_directory (pathlib.Path): The folder prefix where incoming files.json
        ...
        output_directory (pathlib.Path | str): The directory to output the package.
            Defaults to a folder named with the job.json's ID (a random UUID) in the
        ...
        source_copy (bool): When a job.json is completed, a copy of the
            media file as it was uploaded is created in the output directory
            as "source". This can be disabled.
        log_level (StirlingLoggerLevel): set the log level for the job.json. Defaults to INFO.
...
class StirlingJob(StirlingClass):
    ...
    necessary to start up a Stirling job.json. These include things like
    input and output file and directories, and other options related to the
    running of the job.json.


    Attributes:
        source (str): The input source filename (required)
        id (uuid.UUID): The unique ID of the job.json. We create a random UUID v4
            whenever a job.json is created, however a custom one can be supplied so
            long as it is a UUID v4.
        time_start (datetime.datetime): The start time of the job.json in UTC.
        time_end (datetime.datetime): The end time of the job.json in UTC.
        duration (float): the length of the job.json run time, in seconds.
        ...
        plugins (list): A list of plugins to run on the source file. To attach
            Plugins to the job.json, use the add_plugins() method.
        ...
        outputs (list): Once all commands are run and the
            job.json is ready to be closed, the actual output from each of the commands
            will be checked against the expected outputs (here) to ensure that al
            of the expected files.json were generated and the commands ran successfully.
            Note: A job.json will not fail if there are missing or extra files.json generated,
        ...
        framework (core.StirlingMediaFramework): The framework to use to process media
            files.json. Defaults to FFMpeg.
        options (StirlingJobOptions): An object containing the configuration for the
            job.json. This is created from the StirlingConfig object, and is passed to
            the framework when the job.json is created. Variables include:
    ...
    def __post_init__(self):
        """Set up the job.json after it is created.

        The __post_init__ function is called after the object is created. Here,
        we attempt to set up the job.json by doing things like creating the output
        ...
        self._logger.info("Starting job.json.")
        ...
        # Check if the framework requested for the job.json is available.
        self._logger.info(
        ...
        # The source file is now ready for processing, and the job.json has finished loading.
        self._logger.info(
    ...
    def close(self) -> None:
        """Close out the job.json and update its metadata"""
        ...
        self._logger.info(
            f"Ending job.json {self.id} at {self.time_end}, "
    ...
    def run(self) -> None:
        """Run all the commands in the job.json."""
    ...
    def write(self) -> None:
        """Make a snapshot of the job.json (in JSON format) to the job.json log file."""
    ...
    def get_plugins(self) -> List[StirlingPlugin]:
        """Get all the plugins for the job.json."""
    ...
    def add_plugins(self, plugins: List[StirlingPlugin] | StirlingPlugin) -> None:
        """Add new plugins to the job.json.

        Call this It is not currently possible to remove plugins from a job.json once they
        have been added.

        Args:
            *args: A single plugin, or list of plugins to add to the job.json.
        """
    ...
    def _get_source(self, source: Path | None) -> Path:
        """Get the source file for the job.json.

        The source file is the only required field for a Stirling Job. If
            it doesn't exist, or is inaccessible, then the job.json will fail.

        Args:
            source (str): The source file for the job.json.
        """
    ...
    def _get_output_directory(self):
        """Get the full path to the output directory for this job.json.
        ...
        # Make sure we have a directory for the output files.json
        assert self.options.output_directory.is_dir(), AssertionError(
            f"could not find the path {str(self.output_directory)} \
                for output files.json for job.json {str(self.id)}"
        )

        # Make sure we can write to the directory for the output files.json
        assert os.access(self.options.output_directory, os.W_OK), AssertionError(
            f"could not write to path {str(self.output_directory)} \
                for output files.json for job.json {str(self.id)}"
        )

        # Make sure we can write to the directory for the output files.json by
        ...
        with open(str(self.options.log_file), "a", encoding="utf-8") as test_file:
            assert test_file.writable(), AssertionError(
                f"could not write to the log file the path \
                    {str(self.options.log_file)} for job.json {str(self.id)}"
            )

    def _parse(self):
        """Parse the job.json and plugins to generate a list of commands to run."""
